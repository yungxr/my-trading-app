...
export default function TrendingPairs({ pairs, onPairClick, isLoading }: TrendingPairsProps) {
  return (
    <div>
      {isLoading ? (
        Array(5).fill(0).map((_, i) => (
          <div key={i} className="flex justify-between items-center py-2 px-3 mb-1">
            <div>
              <div className="animate-pulse bg-white/10 rounded h-4 w-3/4 mb-1"></div>
              <div className="animate-pulse bg-white/10 rounded h-3 w-1/2"></div>
            </div>
            <div className="animate-pulse bg-white/10 rounded h-4 w-8"></div>
          </div>
        ))
      ) : (
        pairs.map((pair) => (
